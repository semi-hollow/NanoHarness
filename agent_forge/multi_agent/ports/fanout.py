"""COMMON Fanout Application 依赖的外部能力契约。"""
...
# region 2. Worker 与 Finalizer Ports：执行端可替换但语义固定
...
# endregion 2. Worker 与 Finalizer Ports 结束


# region 3. 事件流 Port：与 Runtime 共用同一 Trace sink
class FanoutEvents(EventSink, Protocol):
    """别名，强调 fanout 与 Runtime 共用同一事实流端口。"""
# endregion 3. 事件流 Port 结束
