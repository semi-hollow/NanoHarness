...
from .scorecard import load_benchmark_scorecard
...
def write_ablation_comparison(
    ...
    *,
    ...
    """Load two run scorecards and write paired ablation artifacts."""

    control = load_benchmark_scorecard(control_dir)
    treatment = load_benchmark_scorecard(treatment_dir)
    ...
    for row in comparison.get("paired_cases", []):
        lines.append(
            "| `{instance}` | `{control}` | `{treatment}` | {patch_delta} | {failed_delta} | `{outcome}` |".format(
                ...
                control=row.get("control", {}).get("official_evaluation_status", "not_evaluated"),
                treatment=row.get("treatment", {}).get("official_evaluation_status", "not_evaluated"),
                patch_delta=row.get("delta", {}).get("patch_generated", 0),
                failed_delta=row.get("delta", {}).get("failed_tool_calls", 0),
                outcome=row.get("outcome", ""),
            )
        )
    lines.extend(["", "## Conclusion", "", comparison.get("conclusion", ""), "", "## Limitations", ""])
    lines.extend(f"- {item}" for item in comparison.get("validity", {}).get("limitations", []))
    lines.append("")
    return "\n".join(lines)
