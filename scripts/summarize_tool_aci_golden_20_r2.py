#!/usr/bin/env python3
"""汇总固定 Tool / ACI Golden-20 的 R0/R2 开发集证据。
...
def _git_revision(project_root: Path, revision: str = "HEAD") -> str:
    completed = subprocess.run(
    ...
    if completed.returncode != 0:
        raise ReportRefused(f"cannot resolve Git revision: {revision}")
...
def _usage_metrics(usage: dict[str, Any], trace: dict[str, Any]) -> dict[str, Any]:
    ...
    available_list_files_contexts = sum(
        "list_files" in (context.get("available_tools") or [])
        for context in context_events
        if isinstance(context, dict)
    )
    output_window_observations = sum(
        event.get("event_type") == "tool_observation"
        and isinstance(event.get("observation"), str)
        and "output_chars=" in event["observation"]
        and "output_truncated=" in event["observation"]
        for event in trace.get("events", [])
    ...
    return {
        ...
        "available_list_files_contexts": available_list_files_contexts,
        "output_window_observations": output_window_observations,
        "head_tail_observations": head_tail_observations,
    }


def _failed_tools(usage: dict[str, Any]) -> dict[str, int]:
    efficiency = usage.get("tool_efficiency")
    by_tool = efficiency.get("by_tool") if isinstance(efficiency, dict) else None
    if not isinstance(by_tool, dict):
        raise ReportRefused("usage tool breakdown is missing")
    return {
        str(tool): int(item.get("failed") or 0)
        for tool, item in sorted(by_tool.items())
        if isinstance(item, dict) and int(item.get("failed") or 0) > 0
    }


def _changed_files(patch: bytes) -> list[str]:
    paths: list[str] = []
...
def _load_run(
    ...
    for result in case_results:
        ...
        cases[instance_id] = {
            ...
            "files_changed": _changed_files(candidate),
            ...
            "metrics": metrics,
            "failed_tool_calls_by_tool": _failed_tools(usage),
            "artifacts": {
...
def _decision(r2_resolved: int, regressions: int, activated: bool) -> str:
    if not activated:
        return "invalid"
    if r2_resolved >= 17 and regressions == 0:
        return "strong_positive"
    if r2_resolved == 16 and regressions == 0:
        return "accept"
    if r2_resolved == 15 and regressions == 0:
        return "weak_positive"
    if r2_resolved > 14 and regressions > 0:
        return "mixed"
    return "reject"
...
def _aggregate_variant(
    ...
    metrics.update(
        {
            "patch_bytes": sum(case["patch_bytes"] for case in ordered),
            "files_changed": sum(len(case["files_changed"]) for case in ordered),
        }
    )
    failed_tools: Counter[str] = Counter()
    for case in ordered:
        failed_tools.update(case["failed_tool_calls_by_tool"])
    return {
        ...
        "stop_reasons": dict(
            sorted(Counter(case["stop_reason"] for case in ordered).items())
        ),
        "failed_tool_calls_by_tool": dict(sorted(failed_tools.items())),
        "provider": provider,
        "metrics": metrics,
...
def build_report(
    ...
    if len(r0_dirs) != 4 or len(r2_dirs) != 4:
        raise ReportRefused("exactly four run directories are required per variant")
    runs: dict[str, list[dict[str, Any]]] = {"tool-r0": [], "tool-r2": []}
    for variant, directories in (("tool-r0", r0_dirs), ("tool-r2", r2_dirs)):
        ...
        transitions.append(
            {
                ...
                "r2_patch_sha256": r2["cases"][instance_id]["patch_sha256"],
            }
        )
    gains = counts["unresolved_to_resolved"]
    regressions = counts["resolved_to_unresolved"]
    activation = {
        "find_files_exposed_contexts": r2["metrics"]["available_find_files_contexts"],
        "find_files_calls": r2["metrics"]["find_files_calls"],
        "list_files_exposed_contexts": r2["metrics"]["available_list_files_contexts"],
        "list_files_calls": r2["metrics"]["list_files_calls"],
        "repo_outline_contexts": r2["metrics"]["repo_outline_contexts"],
        "validation_output_window_observations": r2["metrics"][
            "output_window_observations"
        ],
        "validation_head_tail_observations": r2["metrics"]["head_tail_observations"],
        "grep_search_calls": r2["metrics"]["grep_search_calls"],
    }
    activated = (
        activation["find_files_exposed_contexts"] > 0
        and activation["list_files_exposed_contexts"] == 0
        and activation["list_files_calls"] == 0
        and activation["repo_outline_contexts"] == 0
        and activation["validation_output_window_observations"] > 0
    ...
    for name, selected in (
        ...
        after = sum(r2["cases"][item]["resolved"] for item in selected)
        subsets[name] = {
            "planned": 10,
            "r0_resolved": before,
            "r2_resolved": after,
    ...
    return {
        "schema_version": 1,
        "artifact_type": "tool_aci_golden_20_r0_r2_development_comparison",
        "status": "completed",
        ...
        "paired": {
            ...
            "net_resolved_delta": r2["official_resolved"] - r0["official_resolved"],
            "percentage_point_delta": round(
                100 * (r2["official_resolved_rate"] - r0["official_resolved_rate"]), 3
            ),
            "mcnemar_exact_two_sided_p": _mcnemar_exact(gains, regressions),
            "decision": _decision(r2["official_resolved"], regressions, activated),
            "subsets": subsets,
        },
...
def render_markdown(report: dict[str, Any]) -> str:
    ...
    metrics = [
        ...
        ("Files changed (case-sum)", "files_changed"),
    ...
    lines = [
        ...
        f"- Tool-R2：**{r2['official_resolved']}/20** official resolved。",
        (
            "- Tool-R0 / R2 比例与 Wilson 95% 区间："
            f"**{100 * r0['official_resolved_rate']:.1f}% "
            f"[{100 * r0['official_resolved_wilson_95'][0]:.1f}%, "
            f"{100 * r0['official_resolved_wilson_95'][1]:.1f}%]** / "
            f"**{100 * r2['official_resolved_rate']:.1f}% "
            f"[{100 * r2['official_resolved_wilson_95'][0]:.1f}%, "
            f"{100 * r2['official_resolved_wilson_95'][1]:.1f}%]**。"
        ),
        ...
        f"- Frozen run source：`{report['identity']['run_source_commit']}` (`{report['identity']['run_source_tag']}`)",
    ...
    for item in paired["transitions"]:
        lines.append(
            ...
            f"{item['r0']} | {item['r2']} | `{item['transition']}` |"
        )
    lines.extend(
        [
            "",
    ...
    lines.extend(
        [
            ...
            f"- `list_files`：SWE-bench Context 可见 `{activation['list_files_exposed_contexts']}` 次，实际调用 `{activation['list_files_calls']}` 次。",
            f"- `repo_outline`：进入 `{activation['repo_outline_contexts']}` 个 Context（必须为 0）。",
            f"- Validation output window：进入 `{activation['validation_output_window_observations']}` 次；其中真实截断并保留 head/tail `{activation['validation_head_tail_observations']}` 次。",
            "- `apply_patch` 按预注册协议 defer，没有进入 Treatment 变量。",
            "",
            "## 工程判断",
            "",
            "- 过程指标仅用于解释探索深度和效率，不作为 correctness proxy。",
            f"- 预注册门禁裁决为 `{paired['decision']}`；是否保留 R2 以该门禁为准。",
            "",
            "## 证据边界",
            "",
            ...
            f"- Provider 内建 transport retry 按冻结配置最多 2 次；R0/R2 分别观察到 {r0['provider']['retried_calls']}/{r2['provider']['retried_calls']} 个 retried calls，均无 fallback。",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--r2-run-dir", action="append", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = build_report(
        ...
        [_resolve(PROJECT_ROOT, item) for item in args.r2_run_dir],
    )
