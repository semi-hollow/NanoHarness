...
项目不宣称替代 LangGraph、Claude Code 或 OpenCode。成熟框架适合快速交付通用 workflow；
...
```bash
# 浏览真实 run、artifact、Single/Multi timeline 和 Benchmark evidence
forge ui

# 不依赖在线模型，走正式 Runtime 的持久化暂停与恢复链
forge showcase hitl start
forge showcase approval start

# 查看 Smoke-5 从 300 个 SWE-bench Lite case 中怎样选择
...
1. [`Harness.run/resume`](agent_forge/harness.py)：外部稳定接入面。
...
4. [`LiveFanoutCoordinator.run`](agent_forge/multi_agent/application/live_fanout.py)：隔离 worker、DAG 与冲突门禁。
5. [`run_benchmark_campaign`](agent_forge/bench/application/campaign.py)：重复实验与证据聚合。

项目为何演进成当前形态，见[工程演进史](docs/PROJECT_EVOLUTION.md)。
...
| Context 与 Memory | 分区预算、安全压缩、working/checkpoint/long-term 分层 | 不恢复 KV Cache，不声称 vector RAG |
...
| Isolation 与 Multi-Agent | local/worktree/OCI、顺序 artifact handoff、DAG fanout | 本机 coordinator，不是 distributed swarm |
...
详细命令使用 `forge --help`、`forge run --help` 和 `forge bench --help` 查看，避免 README
成为会过期的命令手册。项目名是 NanoHarness；兼容保留的 import package 为
`agent_forge`，CLI 为 `forge`。
...
```mermaid
flowchart LR
    Entry["CLI / Harness / Workbench"] --> Loop["AgentLoop"]
    Bench["SWE-bench / Campaign"] --> Loop
...
API 和 composition root。架构依赖约束由测试执行，不只写在图里。
...
- [工程演进史](docs/PROJECT_EVOLUTION.md)：为什么形成当前架构。
