...
[架构导览](docs/架构导览.md) 是统一架构与导航入口；先在其中建立主链，再按问题进入唯一 owner 文档：

| 问题 | 入口 |
| --- | --- |
| 架构与导航从哪里开始 | [架构导览](docs/架构导览.md) |
| Runtime 全局主链 | [架构导览](docs/架构导览.md) |
| Session、PreparedTurn 与 LLM 入参是什么关系 | [Agent 运行数据结构与模型输入](docs/Agent运行数据结构与模型输入.md) |
| 模型每轮看到什么 | [上下文工程](docs/上下文工程.md) |
| Conversation 如何压缩、长任务如何演进 | [上下文压缩与长任务设计](docs/上下文压缩与长任务设计.md) |
| ToolCall 如何治理和执行 | [运行治理与工具执行](docs/运行治理与工具执行.md) |
