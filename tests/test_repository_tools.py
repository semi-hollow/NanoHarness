...
class RepositoryDiscoveryToolsTest(unittest.TestCase):
    ...
    def test_grep_on_one_file_keeps_repository_relative_filename(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            workspace = Path(tmp)
            ...
            observation = GrepSearchTool(WorkspaceSandbox(workspace)).execute(
                {"keyword": "needle", "path": "src/module.py"}
            )

        self.assertTrue(observation.success, observation.content)
        self.assertIn("src/module.py:1:VALUE", observation.content)
