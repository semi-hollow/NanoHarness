...
该命令创建 `.venv`、安装开发依赖、执行 `forge doctor`，并按源码 symbol 自动安装 13 个
...
1. Dataset 按 `instance_id` 取 case。
2. Checkout `base_commit`，Agent 只看 issue 与代码，不看 gold patch。
3. `LocalCaseExecutor.run` 驱动同一 Runtime 生成 candidate patch。
4. `read_local_validation` 读取 Agent 实际执行过的 focused test 证据。
5. `SwebenchOfficialEvaluator.evaluate` 调独立官方 Oracle，形成 official outcome。
