...
NanoHarness 是本地软件工程智能体，可以在 workspace 中读写文件、应用 patch
和运行已批准 command，因此即使只用于本地开发，也必须明确 security boundary。

## 当前安全边界

完整工具治理分层见[工具治理与执行](docs/工具治理与执行.md#overview)；本文只列
真正实施限制的安全边界：
...
hostile multi-tenant boundary。File tool 仍在 host NanoHarness process 中运行，
...
仓库，除非 NanoHarness 错误处理其 response 或绕过了自身 policy。
