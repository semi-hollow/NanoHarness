...
find_project_dir() {
  ...
  if [ -d "${start_dir}/agent-forge" ] && [ -f "${start_dir}/agent-forge/pyproject.toml" ]; then
    printf '%s\n' "${start_dir}/agent-forge"
    return 0
  fi

  local probe="${start_dir}"
  while [ "${probe}" != "/" ]; do
    if [ -d "${probe}/agent-forge" ] && [ -f "${probe}/agent-forge/pyproject.toml" ]; then
      printf '%s\n' "${probe}/agent-forge"
      return 0
    fi
    probe="$(dirname "${probe}")"
...
main() {
  ...
  PROJECT_DIR="$(find_project_dir)" || {
    log "Could not find NanoHarness/agent-forge from $(pwd)."
    log "Run this script from agent-forge, NanoHarness, or a parent directory containing NanoHarness/agent-forge."
    exit 1
  }
  cd "${PROJECT_DIR}"
  log ""
