...
| SWE-bench-shaped runner | Green | 加载 Case、checkout base commit、运行 Agent、写 Patch 和 `predictions.jsonl`；支持安全 Case manifest、数据 revision、完整计划分母和 per-case Evidence。`forge bench cases/case` 默认隐藏 test/gold Patch。 | Infrastructure Smoke-5 只检查链路；Canonical-50 只代表其确定性 50 题样本。没有 official harness 时，不声称 official resolved。 | `agent_forge/bench/application/swebench.py`、`application/case_inspection.py`、`presentation/case_inspection.py` |
...
| 固定样本质量实验与 Campaign runner | Green | `forge bench campaign` 可固定 cohort、模型、资源配置、安全与执行环境，并以原子 checkpoint 保存槽位进度。当前 `showcase-quality-v1` 使用已见 Golden-10 做有界候选选择，然后仅在预封存 Canonical-50 上运行 Pass@1。 | Golden-10 不是公开质量分数；Canonical-50 不是完整 Verified 排行榜结果。当前两阶段都尚未完成，不预填分数；早期低预算和被拒绝实验只保留在历史归档。 | `agent_forge/bench/application/campaign.py`、`benchmarks/showcase/canonical-showcase-v1.json`、`benchmarks/showcase/quality-selection-protocol-v1.json` |
