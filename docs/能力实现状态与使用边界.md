...
| 操作状态表（代码名 `OperationLedger`） | Green | 需要防重复保护的持久状态变更操作具有稳定 operation key、执行前/后 fingerprint、已执行结果回填和目标漂移检查；target 已在 post-state 时复用旧 Observation，结果未知或漂移时 fail closed。 | 不是 LLM Plan、Run Checkpoint、exactly-once 或分布式事务日志；不能据此重放不可观测的外部副作用。历史重放扩展候选已因回归被拒绝并保留在归档中。 | `agent_forge/runtime/application/operation_tracker.py`、`agent_forge/runtime/adapters/operation_ledger_json.py` |
...
| SWE-bench-shaped runner | Green | 加载 Case、checkout base commit、运行 Agent、写 Patch 和 `predictions.jsonl`；支持安全 Case manifest、数据 revision、完整计划分母和 per-case Evidence。当前公开质量观测是固定 10 题开发样本的 Pass@1 official resolved `4/10`（约 40%）。`forge bench cases/case` 默认隐藏 test/gold Patch。 | `4/10` 只代表该固定开发样本，不外推为完整 SWE-bench Verified，也不改写成尚未运行的 `40/100`。没有 official harness 时，不声称 official resolved。 | `agent_forge/bench/application/swebench.py`、`application/case_inspection.py`、`presentation/case_inspection.py` |
...
| 固定样本质量实验与 Campaign runner | Green | `forge bench campaign` 可固定 cohort、模型、资源配置、安全与执行环境，并以原子 checkpoint 保存槽位进度。当前 Showcase 发布固定开发样本 `4/10`（约 40%）的 official 观测；下一轮是预冻结、按仓库分层的 100 题确认实验。 | 当前数字是系统级样本结果，不隔离模型与 Harness 的独立增益。100 题未运行前不预填结果；早期低预算、候选比较和被拒绝 Treatment 只保留在历史归档。 | `agent_forge/bench/application/campaign.py`、`benchmarks/showcase/canonical-showcase-v1.json`、`docs/evaluation/回归测试与评测范围.md` |
