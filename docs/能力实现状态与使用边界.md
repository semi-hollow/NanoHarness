...
| 操作状态表（代码名 `OperationLedger`） | Green | 需要防重复保护的持久状态变更操作具有稳定 operation key、执行前/后 fingerprint、已执行结果回填和目标漂移检查；target 已在 post-state 时复用旧 Observation，结果未知或漂移时 fail closed。曾评测“同 Run、完整指纹恢复 precondition 后允许一次重放”的 68 行候选，但 Golden Gate 回归，未作为默认能力采纳。 | 不是 LLM Plan、Run Checkpoint、exactly-once 或分布式事务日志；不能据此重放不可观测的外部副作用。Case-level 正向不能覆盖 Golden 回归或证明该候选可默认上线。 | `agent_forge/runtime/application/operation_tracker.py`、`agent_forge/runtime/adapters/operation_ledger_json.py` |
...
| 固定样本质量实验与 Campaign runner | Green | `forge bench campaign` 可固定 cohort、模型、预算、安全与执行环境，并以原子 checkpoint 保存槽位进度。机器摘要分开保留 Phase-1 R0-R3（全部拒绝/回滚）和 Phase-2：Target `0/2 U -> 2/2 R`、Guard `3/3 R`，但唯一 Golden 扩跑为 `4 R / 5 U / 1 empty`，原 resolved 仅保留 `3/5`，另有 1 次 Provider infra；因此默认 Treatment 拒绝并已由可审计 revert `042846a` 回滚，Goal/Gate 4 未完成。 | 两阶段 Provider/model 不同，不能直接 A/B；Target 是事后选择的 Case-level 证据，Guard 只说明所测范围未观察到退化。Golden comparison protocol-invalid，且 correctness 也净退化；不能声称 accepted、non-regression、总体 uplift、随机稳定性或 Goal 完成。 | `agent_forge/bench/application/campaign.py`、`benchmarks/runtime-quality/golden-10-v1.json`、`docs/evaluation/功能冻结后的Runtime质量实验.md` |
