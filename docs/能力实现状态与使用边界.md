...
| 固定样本质量实验与 Campaign runner | Green | `forge bench campaign` 可固定 cohort、模型、温度、预算、安全与执行环境，并以原子 checkpoint 保存槽位进度。当前质量主线使用 Golden-10 与 Sentinel-4，按 R0/R1/R2 记录候选版本、逐题语义哨兵和采纳决策；Workbench 读取机器摘要展示结果。 | Golden-10 是单次开发实验，不外推排行榜或随机稳定性；历史 50×2 preset 同时改变 routing 与 Skill，只作为负实验制品，不承担当前因果结论。 | `agent_forge/bench/application/campaign.py`、`benchmarks/runtime-quality/golden-10-v1.json`、`docs/evaluation/功能冻结后的Runtime质量实验.md` |
