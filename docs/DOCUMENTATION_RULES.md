...
正文使用正向执行叙事：先给系统角色和一个端到端 Case，再顺着真实状态转换下钻。
“为什么不用某方案”等反事实内容不作为章节骨架；真正影响 contract 的限制只在能力文档
末尾用一小段 Design Boundary 收口，跨能力扩展统一归 `生产化边界与扩展.md`。
...
标题优先命名正在发生的动作或状态 Owner，例如“候选集成”“崩溃恢复”。避免使用
“为什么不……”或“为什么已经足够”等答辩式标题，以免把非主线方案提前引入正文。
...
6. 是否先出现真实主链，再出现边界和扩展？
7. 标题是否在讲当前设计，而不是主动提出反事实问题？
