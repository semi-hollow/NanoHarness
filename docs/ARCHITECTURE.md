...
README 只负责项目定位与使用入口；所有可调用动作集中在
`docs/architecture/facade-catalog.md`；重要决策记录在 `docs/adr/`。当其他文档与本文冲突时，
以本文和可执行架构测试为准。
...
| Single-Agent Runtime construction | `Harness.run` 是 run-level owner；`runtime.wiring` 是低层装配 owner |
...
所有公开 Single-Agent 入口都委托 `Harness`，不得复制 trace、environment、AgentLoop、artifact 或
cleanup 生命周期。Advanced worker 可以复用内部 wiring，但不能成为第二个 public run owner；
Single-Run 的 `inspect` 与 Workbench 读取 canonical Run Story；Benchmark/Campaign 读取各自的
类型化结果与 Scorecard。展示层都不得从原始文件重新推断 Domain 状态。
...
简单能力保持单文件或扁平目录；复杂能力才按 `api / domain / application / ports / adapters /
wiring` 拆分。只有流程与外部副作用混杂、变化原因明显不同、业务规则无法脱离基础设施测试，
或外围开始越层导入时，才增加层；仍须通过 1.1 的收益与预算检查。

| 层 | 可以拥有/依赖 | 禁止 |
| --- | --- | --- |
| Domain | 标准库、同能力纯类型、状态与规则 | Application、Adapter、文件/网络/Git/进程 |
| Application | Domain、Port、用例编排、恢复/错误语义 | 创建具体 Adapter、任意 I/O、渲染报告 |
| Ports | Application 真正需要的最小 `Protocol` | 为纯函数、私有 helper 或假想实现机械建 Port |
| Adapters | 外部解析、I/O、重试、超时、原子写入和清理 | 决定任务成功、恢复或 official truth |
...
- Public CLI 按使用频率分层：Core 为 `run / inspect / demo`，Operator 为 `resume`，Advanced
  为 `bench / ui`。同义 Legacy 命令直接删除；`eval / memory / skills / doctor / MCP`
  作为非同义内部能力隐藏，不进入 README、Core Scope 或主演示。
