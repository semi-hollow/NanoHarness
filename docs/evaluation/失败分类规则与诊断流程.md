...
它相当于确定性的故障归因服务：运行事实产生后，再根据状态、外部回执和异常码归入唯一失败类型；
而不是让另一个模型阅读日志后自由发挥。
...
| `unsafe_or_blocked_command` | Command/permission policy 阻断了不安全 action。 | 改用受限工具或 allowlist 内命令；人工批准不能覆盖明确的 `DENY`。 |
...
| `input_policy_block` | Trace 明确记录 Task 在第一次模型调用前被输入策略阻断。 | 检查是否把引用内容误当成可执行动作；当前风险词扫描本身只记证据。 |
...
工具类故障按[工具治理主视图](../运行生命周期与异常处理机制.md)定位；分类名称描述故障事实，
不等于负责处理它的某个 Hook 或代码文件。
