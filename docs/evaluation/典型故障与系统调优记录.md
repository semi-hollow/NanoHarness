...
当前公开质量入口是 [Quality Showcase](../../benchmarks/showcase/canonical-showcase-v1.json)，而不是下文任何一轮历史
Treatment。公开面先给结果，再给证据边界：

| 维度 | 当前制品 | 状态与结论边界 |
|---|---|---|
| 当前质量观测 | 固定开发样本 | single Agent、Pass@1、official resolved `4/10`，约 40% |
| 扩样本确认 | 100-Case | 预冻结、按仓库分层后只运行一次；完成前不写成 `40/100` |
...
`4/10` 是当前系统在该固定样本上的观测，不声称完整 Verified 总体率，也不隔离模型与 Harness 的独立增益。
...
当前公开质量结论只读取 Quality Showcase 中已完成的固定开发样本；未来 100 题只有在预封存成员、协议和
完整终态后才能替换该句。
...
不改写当前 `4/10` 质量观测，也不预填未来 100 题确认结果。
