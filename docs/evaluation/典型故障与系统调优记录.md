...
随机样本，也不能作为 500 题总体解决率。少量早期试运行只能说明已运行槽位的事实。
...
搜索工具曾同时注册 `grep` 和 `grep_search` 两个同义名称；SWE-bench 路由还隐藏了异构仓库需要的
受限验证回退。Governed 在
...
**触发现象**：50×2 实验中有 39 个 Run 以 `pending_tool_call_at_stop` 结束且没有
Candidate Patch。初步容易归因为模型不会改或 Tool 不够。

**排除误判的证据**：39/39 全部刚好用完当时的 16 次模型调用上限，35/39 已触发
Context Compaction。34 个 official resolved Run 的平均模型调用数是 11.24，其中也有
10 个走到第 16 轮。这说明 16 不是从任务收敛分布得出的边界，而是早期默认值。

同时，Benchmark Workspace 位于 `.agent_forge/.../workspace`；旧 Repo Map 在绝对路径上检查
`.agent_forge` 忽略规则，会把整个仓库判为应忽略。修复前该路径返回 `0 files / 0 chars`；
修复后在已缓存 pytest 仓库上返回 652 个相对路径。DeepSeek V4 的物理 Context Window
还曾被默认为 32K，与 Runtime 愿意使用的 Prompt 策略预算混在一起。

**备选方案**：直接继续调 Prompt/Router/Skill；把 max steps 无上限调大；或先修复仓库可见性和
模型能力声明，再做 16/32 单因素预算敏感性。选择第三种，因为它能区分“被实验系统
截断”和“给予合理预算后仍无法解决”。
...
- `read_file`、`grep_search`、`list_files` 和验证命令的返回值必须标明真实窗口与截断状态，
  不得让模型误以为已看到未返回的内容。
- 默认模型轮数收敛为 32，但是否采纳必须由预注册的 16/32 配对实验决定，不再凭直觉加大。
...
**当前边界**：修测量环境不等于解决率已提升。在 E0 完成前，历史 `20/50` 只是带有预算截断和
空 Repo Map 的旧观察，不作为当前 Runtime 能力分数。
