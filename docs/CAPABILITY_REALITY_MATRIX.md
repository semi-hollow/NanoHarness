...
| Tool governance | Green | Task 文本中的 URL、路径或危险命令示例只形成 trace 提示，不被误当成已授权动作；真正 ToolCall 依次经过 routing、registry validation、permission hook、command policy 和 sandbox。 | 不把 keyword filter、prompt-only safety 或本地模式说成 OS-level isolation。 | `agent_forge/tools/`、`agent_forge/safety/`、`agent_forge/runtime/hooks.py` |
...
| 持久化 human clarification | Green | Pre-loop ambiguity 和模型 `ask_human` 会原子持久化 request，在同一 turn 中优先阻断其他 tool，`RunLifecycle` 将 run 转为 `waiting_human`；Operator Console 可原位回答并自动 continuation，无头入口仍可用 `forge resume --answer`。 | 它只记录信息，不授权副作用，也不恢复隐藏 model state。 | `agent_forge/runtime/application/run_preparation.py`、`tool_execution.py`、`run_lifecycle.py`、`agent_forge/operator_console/session.py` |
| 副作用人工审批 | Green | 写入型 action 在执行前持久化 approval；Operator Console 展示 tool、参数、workspace 和 fingerprint，批准或拒绝后自动 continuation；无头入口仍支持 `forge resume --decision`。Runtime policy 是唯一授权 owner，获准后的工具不重复执行旧式门禁。 | Approval 与 clarification 是不同契约；本地文件 store 不是 multi-user authorization service。 | `agent_forge/runtime/application/tool_authorization.py`、`runtime/adapters/approval_json.py`、`agent_forge/operator_console/session.py` |
...
| Operator Console | Green | `forge console` 通过同一 `Harness.run/resume` 执行 single Agent，实时消费有序 `RuntimeEvent`；同屏支持 steer、协作式 pause/cancel、HITL、写审批与自动 continuation。PyCharm 共享配置使用真实 DeepSeek 与安全练习仓库。 | 它是本地 presentation adapter，不是第二套 Runtime、完整 IDE/TUI 产品或远程多用户控制面；当前模型 transport 不提供 token delta，也不展示或伪造隐藏思维链。 | `agent_forge/operator_console/`、`examples/operator_console.py`、`tests/test_operator_console.py` |
