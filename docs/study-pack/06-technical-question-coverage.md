...
这份文档回答一个问题：你整理的技术问题，哪些能用 Agent Forge 这个项目直接支撑，哪些只能从项目发散，哪些不应该塞进项目代码。

标记说明：

- `代码覆盖`：项目里有明确模块、运行路径或 trace 证据。
- `部分覆盖`：项目体现了核心思想，但没有做完整工业系统。
- `文档延展`：不适合写进 CodingAgent 代码，但技术沟通要能讲。
...
Agent Forge 覆盖的是 CodingAgent runtime 主线：架构、ReAct、上下文工程、tool calling、权限安全、异常恢复、multi-agent 编排、trace/session/eval。RAG/GraphRAG、多模态、训练、C 端产品体验是扩展技术知识，不应强行塞进核心代码。
