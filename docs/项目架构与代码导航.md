# NanoHarness 项目架构与代码导航

本文是 NanoHarness 唯一权威的项目架构契约。它同时回答三类问题：

1. 运行请求如何进入系统并完成一次 Agent 任务。
2. 各模块拥有什么数据、规则和外部状态变化。
3. 新代码必须遵守哪些依赖、类型、状态和测试标准。

README 负责项目定位、Public CLI/API 与使用入口；架构演进和关键取舍记录在
[代码结构演进与可读性治理](architecture/代码结构演进与可读性治理.md)。当其他文档与本文冲突时，
以本文和可执行架构测试为准。

本文用于**定位代码和核对依赖**。运行机制总览见[运行生命周期与异常处理机制](运行生命周期与异常处理机制.md)；
当前策略与可点击代码 Owner 见[核心运行机制与代码索引](核心运行机制与代码索引.md)。稳定主视图为“入口控制 → 执行决策 → 受限执行 → 结果与恢复”。
...
项目不追求完整 IDE、通用工作流平台或微服务拆分，也不机械采用一类一文件。
分层存在的理由是控制变化和依赖，而不是增加目录数量。

### 1.1 复杂度治理硬约束

状态变化受控、Evidence 真实可信是硬底线。在满足底线的设计中，复杂度按以下四条规则
...
但不得增加 Single-Agent 默认入口、主演示或核心阅读范围。
...
| Core Review Scope | 12 个文件；超过 15 个必须先完成合并审计 |
...
文件数和 LOC 不是机械 KPI；数字增加时必须写清新增语义、理解成本和无法合并的原因。CLI parser、
Adapter 序列化、Memory、Skills、MCP、Multi/Fanout、Campaign 和 UI 不计入 Core Review Scope。

当前 12-file Core Review Scope（按运行顺序排列）：
...
六边形架构保留是为了 Port 替换、边界测试和外部交互隔离，不是为了风格纯度。没有独立语义的
层允许合并、内联、私有化、降为 Advanced 或删除。

#### 代码导航约定

任意 Core symbol 必须通过附近代码、类型签名和 `forge inspect` 暴露以下六项导航信息：
...
任意 run artifact 同样提供六项导航信息：kind/path、producer/flow stage、consumer、source/authority、
...
-> before_model 生命周期时机 -> ModelPort.chat -> after_model 生命周期时机
-> 模型返回 final answer：RunLifecycle.finalize_run
-> 模型返回 ToolCall：进入下方四层工具治理
```

工具分支按四层定位；这是职责分层，右列才是真实调用顺序：

| 主视图 | 真实调用顺序 | 核心代码入口 |
| --- | --- | --- |
| 入口控制 | `ToolRouter` 选择可见工具 → `ToolCallNormalizer` 归一化 → `ToolExecutionPipeline` 路由复核 → 放行后由 `ToolRegistry` 做参数契约校验 | `tools/tool_router.py`、`models/tool_call_normalizer.py`、`runtime/application/tool_execution.py`、`tools/registry.py` |
| 执行决策 | `OperationTracker` 建立身份/查操作状态表 → `before_tool` 具体处理器 → `ToolAuthorizationGate` 处理允许、拒绝或人工审批 | `runtime/application/operation_tracker.py`、`runtime/application/tool_authorization.py`、`runtime/hooks.py` |
| 受限执行 | 通过授权门后，具体 Tool 仍受命令规则、工作区路径和执行环境约束 | `safety/command_policy.py`、`safety/sandbox.py`、`runtime/execution_environment.py`、`tools/` |
| 结果与恢复 | 具体 Tool 返回 `Observation` → `after_tool` 最终脱敏 → 更新操作状态表、Trace 和 Checkpoint → 下一 Turn 或终态 | `runtime/application/tool_execution.py`、`runtime/application/operation_tracker.py`、`runtime/application/run_lifecycle.py`、`runtime/hooks.py` |

上表只负责代码定位；Hook 和四层的概念关系由[运行生命周期与异常处理机制](运行生命周期与异常处理机制.md)统一说明。
...
正在执行的 HTTP/进程，也不会自动回滚操作状态表已记录的持久状态变更。实时流提供
...
Benchmark 的完成顺序为 `bench.api.run_swebench -> RunSwebench.run_benchmark` → 执行用例 → 暂存预测 → 可选官方评测 → 生成失败诊断、案例和汇总计分卡。

Evidence 保持单向：`TraceEvent` 事实 → 内部 JSON、脱敏 `RuntimeEvent` 和可选 OTEL 投影 → `RunManifest` 产物血缘 → 标准 `RunStory` → `forge inspect`、Markdown 和 Workbench。
...
wiring` 拆分。只有流程与外部交互混杂、变化原因明显不同、业务规则无法脱离基础设施测试，
...
`class XxxPort(Protocol)` 声明调用方需要的方法形状，属于结构化接口。Python 不会扫描仓库寻找实现；
`wiring.py` 在装配点选择对象，mypy 比较方法签名，运行时仍是普通对象调用。
...
- 附加生命周期处理器不能替换默认的执行环境预检、权限判断和敏感信息脱敏；
  `before_model/before_tool/on_stop` 异常时阻断，`after_model/after_tool/on_checkpoint` 异常隔离并留证。
...
- 持久状态变更操作必须有稳定 operation key、前置 fingerprint 和操作状态表记录。
- 恢复时不得盲目重复已经执行的操作，也不得复用目标状态已变化的旧结果。
- timeout、cancel、provider error、policy denial、validation failure 必须可区分。
- pause/cancel 必须说明安全检查边界和已经发生的持久状态变化；steer 只能追加新的用户方向并丢弃
...
- 可从事实重新生成的 projection 可以删除后重建；Checkpoint、操作状态表或 evaluator authority 等
...
- 旧路径若保留，只是明确标注的兼容导出。
- 本文各项目地图和代码一致，主要入口可从折叠视图识别。
- 新增类型和状态转换有测试。
- Core Scope、Public Entry 和唯一 owner 预算没有隐式增长；增长时已记录独立语义和理解成本。
- 随机 Core symbol 与 artifact 均能按 Navigation Contract 定位六项信息。
- `docs/evaluation/典型故障与系统调优记录.md` 记录代表性问题、根因、修复和验证。
